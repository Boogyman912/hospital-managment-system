...
public class DoctorController {
    ...
    public ResponseEntity<List<FeedbackDTO>> getFeedbacksByDoctorId(Authentication auth)
            throws RuntimeException {
        Long doctorId = userRepository.findByUsername(auth.getName())
                .orElseThrow(() -> new UsernameNotFoundException("Doctor not found")).getDoctorId();
    ...
    public ResponseEntity<String> createPrescription(@RequestBody Prescription prescription,
            Authentication auth, @PathVariable Long appointment_id) throws RuntimeException {
        String doctorUsername = auth.getName();
        User doctor = userRepository.findByUsername(doctorUsername)
                .orElseThrow(() -> new UsernameNotFoundException(
                        "Doctor with username '" + doctorUsername + "' not found"));
        ...
        if (!appointmentService.isAppointmentWithDoctor(appointment_id, doctorId)) {
            return ResponseEntity.status(403)
                    .body("Forbidden: Patient did not book appointment with you");
        }
    ...
    public ResponseEntity<String> updatePrescription(Authentication auth,
        ...
        // The try-catch block for the entire method is generally redundant (see note below),
        ...
        try {
            ...
            Long issuingDoctorId = prescriptionService.getPrescriptionById(prescription_id)
                    .getDoctor().getDoctor_id();

            if (authenticatedDoctorId != issuingDoctorId) {
                // 🚨 COMPLETION: Throw an exception to deny access (HTTP 403 Forbidden)
            ...
            prescriptionService.updatePrescription(prescription_id, medications, labTests,
                    instructions, date_issued);

            return ResponseEntity.ok("Prescription updated successfully");

        } catch (ResponseStatusException rse) {
            // Catch and rethrow ResponseStatusException to ensure the correct status code is used
            throw rse;
            ...
            // Generic fallback for unexpected errors (e.g., service failure, database issue)
    ...
    public ResponseEntity<Map<String, Object>> createSlots(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(
                    Map.of("success", false, "message", "Error creating slots: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> deleteSlotById(@PathVariable Long slotId,
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(
                    Map.of("success", true, "message", "Deleted the slot with slotId " + slotId));
        } catch (Exception e) {
            // TODO: handle exception
            return ResponseEntity.badRequest().body(
                    Map.of("success", false, "message", "Error deleting slot: " + e.getMessage()));
        }
    }
